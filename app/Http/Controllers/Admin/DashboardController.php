...
class DashboardController extends Controller
{
    ...
    public function newdashboard(Request $request){
            ...
            if($request->district || $request->centers || $request->batches){
                    ...
                    foreach ($allcenter as $allcenters) {
                        ...
                        foreach($batchDatass as  $batch){
                            ...
                            $us = DB::table('user_batch')->join('users','user_batch.user_id','=','users.id')->where('batch_id',$batch->id)->groupBy('users.id')->pluck('users.id');
                                ...
                                // print_r( $presenuser); die;

                                // $abcdef = '['.$batch->batch_name .','. $presen . ','. $ab.']';
                                $abcdef = [
                ...
                // print_r($graph); die;
                    return view('admin.newdashboard',compact('districts','countstudentbatch','countdistrict','countcenters','countbatches','batchDatas','batchname','presenuser','graph','absentuser','getcenteruser','districtbatchcenter','all_dates','usersdata','allids'));
            }
                    $all_dates =array();
                    $usersdata= array();
                    $countstudentbatch =0;
                    ...
                    $allids['batches'] = null;
                    $graph =array();
         return view('admin.newdashboard',compact('districts','countstudentbatch','countdistrict','countcenters','countbatches','batchDatas','batchname','presenuser','absentuser','getcenteruser','graph','districtbatchcenter','all_dates','usersdata','allids'));
    }
