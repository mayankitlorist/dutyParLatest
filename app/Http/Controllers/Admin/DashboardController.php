...
use App\Exports\AttendenceSearchData;
    ...
    public function newdashboard(Request $request){
            ...
            if($request->district || $request->centers || $request->batches){
                ...
                $allids['date'] = $request->date;
                    ...
                    array_push( $getcenteruser, $office);
                    foreach($getcenteruser as $getcenterusers){
                               $Cname = DB::table('user_batch')->where('office_id', $request->centers)->first();

                                if($Cname){
                                      $Cusername =  DB::table('users')->where('id', $Cname->center_head)->first(); 
                                     $getcenterusers->centerhead = $Cusername->name;
                    ...
                    foreach($batchDatas as $batchData){
                        ...
                        $offi = DB::table('offices')->where('location',$batchData->location)->first();
                        $Cname = DB::table('user_batch')->where('batch_id', $batchData->id)->whereNotNull('center_head')->first();
                        if($Cname){
                              $Cusername =  DB::table('users')->where('id', $Cname->center_head)->first();
                              // print_r($Cusername) ; die;
                              $batchData->centerhead = $Cusername->name;
                        }else{
                              $batchData->centerhead = null;
                        }
                ...
                $allids['date'] = $request->date;
                    ...
                    foreach ($getcenteruser as $getcenterusers) {
                        ...
                        $getcenterusers->centername = '';
                         ...
                         $Cname = DB::table('user_batch')->where('office_id', $getcenterusers->id)->whereNotNull('center_head')->first();
