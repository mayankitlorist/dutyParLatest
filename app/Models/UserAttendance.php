              ...
              "b" => "64974"
                ...
                if($user->id == 55034){
                    ...
                    $date11 = date('Y-m-d', strtotime($date1. ' + 1 days'));
                        if(!empty($batch)){
                            $data1 = DB::table('user_attendance_history')
                            ->join('users','user_attendance_history.user_id','=','users.id')
                            ->join('batch','user_attendance_history.batch_id','=','batch.id')
                            ->orWhereNotNull('user_attendance_history.batch_id')
                            ->whereIn('users.parent_id',$data)
                            ->whereBetween('user_attendance_history.intime', [$date,  $date11])
                            ->where('user_attendance_history.batch_id',$batch)
                            ->get();
                            return $data1;
                            ...
                            if($date == $date1){
                                // echo "hi ";die;
                                 $data1 = DB::table('user_attendance_history')
                                            ...
                                            ->orWhereNotNull('user_attendance_history.batch_id')
                                            ->whereIn('users.parent_id',$data)
                                            ->whereDate('user_attendance_history.intime',$date)
                                             ->get();
                                             return $data1;
                            }else{
                                // DB::enableQueryLog();
